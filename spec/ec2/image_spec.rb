...
describe Automan::Ec2::Image do
  ...
  describe '#default_image_name' do
    before(:each) do
      allow(subject).to receive(:name).and_return('skee-lo')
    end
  ...
  describe '#image_snapshot' do
    it "returns id if the snapshot exists" do
      subject.ec2.client.stub_responses(
        :describe_images,
        images: [
          image_id: 'foobar',
      ...
      expect(subject.image_snapshot('foobar')).to eq('foo')
    end
    ...
    it "returns nil if the snapshot is nil" do
      ...
      expect(subject.image_snapshot('foobar')).to be_nil
    end
  ...
  describe '#delete_snapshots' do
    it 'does not delete snapshots with state != "completed"' do
      snapshot = double(:snapshot)
      allow(snapshot).to receive(:state).and_return('foo')
      allow(snapshot).to receive(:id)
      expect(snapshot).to_not receive(:delete)
      subject.delete_snapshots( [ snapshot ])
    end

    it 'deletes snapshots with state == "completed"' do
      snapshot = double(:snapshot)
      allow(snapshot).to receive(:state).and_return('completed')
      allow(snapshot).to receive(:id)
  ...
  describe '#deregister_images' do
    before(:each) do
      ...
      allow(subject).to receive(:image_snapshot).and_return('foo')
    end

    it 'does not deregister images with state != "available"' do
      allow(@image).to receive(:state).and_return('foo')
      allow(@image).to receive(:tags).and_return( [{key: 'CanPrune', value: 'yes'}] )
      expect(@image).to_not receive(:deregister)
    ...
    it 'does not deregister images w/o prunable tag' do
      ...
      allow(@image).to receive(:tags).and_return( [] )
      expect(@image).to_not receive(:deregister)
      subject.deregister_images( [ 'foo' ] )
    end
